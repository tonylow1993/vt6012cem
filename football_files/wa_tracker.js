...
try{
	if(!WAGdpr.isEU()){
	try{s.visitor = Visitor.getInstance('06AB2C1653DB07AD0A490D4B@AdobeOrg');} catch(visitorapi){console.log(visitorapi)}
    }
}
...
function s_doPlugins(s) {
   ...
   /* previous page to prop */
	...
	if(typeof(DataLayer)!='undefined'){
		if(DataLayer.abort=='yes'){s.abort=true;}
	}
	if(WATracker.getOptions().enabled===false){s.abort=true;}
}
