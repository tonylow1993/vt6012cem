...
function displayInplayClock(pageName) {
    return (jsInplayPools.indexOf(pageName) != -1 && pageName != "HFT")
...
function goToPage(pageNo) {
    if (pageNo <= 0 || isPaginationClick)
        return;
    let promptBox = pageName != "MIXALLUPLIST" && $(".checkedOdds").length > 0;
...
function formatTVIcon(_tv, _cssClass) {
    if (_tv != null) {
        if (_tv.length > 0) {
            function tvIconLink(para) {
                return <span key={`tvIcon${para[0]}`} onClick={()=>{goTVUrl();}} style={{"cursor": "pointer"}}>
                        <Img src={`${nasImagePath}icon_tv-${para[0]}.gif${cacheVersion}`} alt={para[1]} title={para[1]} />
                    </span>;
            }
...
var Tournament = function(data, tableType) {
    ...
    for(var i=0; i<allOddsType.length; i++) {
        ...
        if(_oddsType!==undefined && _oddsType!==null) {            
            ...
            if(!checkIfRefPool(singleTourn[allOddsType[i].toLowerCase() + "odds"]) || checkIfHybridPool(singleTourn[allOddsType[i].toLowerCase() + "odds"]))
                singleTourn.arrPools.push(allOddsType[i]);
        }
    }
...
var parseIntWith00 = function (str) {
        ...
        if(pageName=="INPLAYALL" || pageName=="RESULT") {
            ...
            if(pageName=="INPLAYALL") {
                ...
                if (('ent' + oddsSuffix) in this && this['ent' + oddsSuffix] !== null && this['ent' + oddsSuffix].length > 0 && hasOdds(this['ent' + oddsSuffix][0].H)) {
                    ...
                    let _found = false;
                    for(let ntsInd=0; ntsInd<this['nts'+oddsSuffix].length; ntsInd++) {
                        if(hasOdds(this['nts'+oddsSuffix][ntsInd].H)) {
                            this['nts'+oddsSuffix] = this['nts'+oddsSuffix][ntsInd];
                            _found = true;
                            break;
                        }
                    }
                ...
                // extend ets to nts
                if (('ets'+oddsSuffix) in this && this['ets'+oddsSuffix]!==null) {
                    if (!(('nts'+oddsSuffix) in this) || this['nts'+oddsSuffix]==null) {
                        this.ntsodds = [];
                    }
                    this.ntsodds = this.ntsodds.concat(this.etsodds);
                }
            }
        ...
        if(singleMatch['spc'+oddsSuffix]!=undefined && singleMatch['spc'+oddsSuffix]!=null && singleMatch['spc'+oddsSuffix].length>0) {
            ...
            if(pageName=="ALL") {
                if(checkIfHybridPool(singleMatch['spcodds'])) {
                    ...
                    //create spcoddsref
                    for(let i=0; i<singleMatch['spcodds'].length; i++) {
                        if(singleMatch['spcodds'][i].Cur=="0") {
                            singleMatch['spcoddsref'].push(singleMatch['spcodds'][i]);
                        }
                    }
...
Match.prototype.isNTSDefined = function () {
    if (this.definedPools.indexOf("NTS") != -1 || this.definedPools.indexOf("ETS") != -1 || this.definedPools.indexOf("ENT") != -1
        || this.inplayPools.indexOf("NTS") != -1 || this.inplayPools.indexOf("ETS") != -1 || this.inplayPools.indexOf("ENT") != -1) {
        return true;
    }
    return false;
};
...
Match.prototype.GetMatchPoolStatus = function(_poolType, _lineNum) {
    ...
    let _singlePoolType = this[_poolType.toLowerCase() + "odds"];
    let _poolInplay, _poolStatus;
    ...
    if(_singlePoolType!=null && _singlePoolType.length!=null) {
        if(_poolType.match(/^(MSP|SGA)$/) ) {
            if(_lineNum==null)
                ...
                _singlePoolType = $.grep(_singlePoolType, function(_item){ return _item.ITEM == _lineNum; })[0];
        } else {
            _singlePoolType = _singlePoolType[0];
        }
    }
...
Match_Result.prototype.GetNTSResult = function(pooltype, _ind) {
    let strvalue = this.results[pooltype+_ind];
    try {
        if(strvalue=="RFD") {
            return jsRFD;
        }
        if (strvalue == "H")
            return "1 " + this.homeTeam["teamName" + jsLang];
        else if (strvalue == "A")
            return "2 " + this.awayTeam["teamName" + jsLang];
...
Match_Result.prototype.GetNonNTSResult = function(pooltype) {
    ...
    try {
        ...
        switch (pooltype) {
            ...
            case "EHA":
                if (strvalue=="H")
                    return jsHomeTeamWin;
                else if (strvalue=="A")
                    return jsAwayTeamWin;
                else if (strvalue=="D")
                    return jsDRAW;
                break;
            case "CRS":
            ...
            case "ECS":
                ...
                if (strvalue == "HO" || strvalue.indexOf("-H") > 0)
                    ...
                    return (parseInt(score[0], 10) + " : " + parseInt(score[1],10));
                break;
            case "HFT":
            ...
            case "TQL":
                if (strvalue == "H")
                    return this.homeTeam["teamName"+jsLang];
                else if (strvalue == "A")
                    return this.awayTeam["teamName"+jsLang];
                break;
            case "FGS":
...
Match_Result.prototype.GetFGSResultDetailsText = function(isFgsResultPage) {
    try {
        ...
        for (var i = 0; i < _allResult.length; i++) {
            ...
            if (_sel == '000')
                _sel = '00';
            _matchFGSText.push(<div key={`${this.matchID}_result_${_sel}`}>{_sel} {(jsLang == "CH" ? _allResult[i].CONTENTCH : _allResult[i].CONTENTEN)}</div>);
        }
  ...
  for(var i=0; i<data.length; i++) {
   ...
   //   if(noOfPassedMatch + getMatchLength(data[i])>=startMatch && noOfPassedMatch<endMatch) {
      if ( noOfSavedMatch < endMatch - startMatch -1 ) {
        let arrstartMatch = startMatch - noOfPassedMatch - 1 + noOfSavedMatch;
...
function getResultScoreDetails(singleMatch) {
    ...
    if (singleMatch.accumulatedscore == null) {
        ...
        let firstHalfResult = $.grep(singleMatch.accumulatedscore, function (_as) { return _as.periodvalue == "FirstHalf"; })[0];
        let secondHalfResult = $.grep(singleMatch.accumulatedscore, function (_as) { return _as.periodvalue == "SecondHalf"; })[0];
        let extraTimeResult = $.grep(singleMatch.accumulatedscore, function (_as) { return _as.periodvalue == "ExtraTime"; })[0];
        let isVoid = singleMatch.isVoidMatch();
        if (isVoid && singleMatch.matchState == 'ResultVoided') {
            if (firstHalfResult == null || firstHalfResult.periodstatus != "ResultFinal") {
                ...
                secondHalfScore = <div className="void">{jsvoidmatch}</div>;
            }
        }
        else {
            if (firstHalfResult == null || firstHalfResult.periodstatus != "ResultFinal") {
            ...
            if (secondHalfResult == null || secondHalfResult.periodstatus != "ResultFinal") {
                if (isVoid) {
                    if (pageName == "DHCP_RESULT" || pageName == "HFMP_RESULT") {
                        secondHalfScore = <div className="void">{jsvoid_match}</div>;
                    } else {
                        secondHalfScore = <div className="void">{jsvoidmatch}</div>;
                    }
                }
            ...
            if (extraTimeResult != null) {
                if (isVoid && singleMatch.matchState == 'InPlayResultVoided' && secondHalfResult != null && secondHalfResult.periodstatus == "ResultFinal") {
                    extraTimeScore = <div className="lblExtraTime void">[{jsvoidmatch}]</div>;
                }
                else if (extraTimeResult.periodstatus == "ResultFinal") {
                    extraTimeScore = <div className="lblExtraTime">[{extraTimeResult.home} : {extraTimeResult.away}]</div>;
                }
        ...
        try {
            if (isLogon() && hasWebTV(match.matchID) && GetDataStore('football_live_ind') == "Y") {
                ...
                if (match.IsMatchKickOff()){  
                    return <a href="javascript:;" onClick={function () { top.openWebTVWindow(match.matchID) }}>
                        <img src={srcUrl} title={jsfootballLiveSchedule} onerror='errImg(this);' /></a>;
                }else{ 
                   
                    return <a href="javascript:;" onClick={function () { top.openWebTVWindow() }}>
                        <img src={srcUrl} title={jsfootballLiveSchedule} onerror='errImg(this);' /> </a>;
                }
            }
        }
...
function getLiveCastStr(match) {
    ...
    if (match != null && match.liveEvent != null && match.liveEvent.matchIDbetradar != null && match.liveEvent.matchIDbetradar != "") {
        return <a href="javascript:;" onClick={() => { callLiveCast(match.liveEvent.matchIDbetradar) }}> <img src={srcUrl} title="" onError={() => { errImg(this) }}/></a>;
    } else {
        return null;
    }
...
class PerformTVLink extends React.Component {
    render() {
        ...
        if (performTvEnabled.indexOf(pageName) != -1) {
            return <a style={linkPerformTvStyle} id="linkPerformTv" target="_blank" onClick={() => { top.openWebTVWindow(); return false; }}>
                <Img src={`/football/info/images/tv_title.gif${cacheVersion}`} title="HKJC Football Live Schedule" />
...
class HelpIcon extends React.Component {
    render() {
        return <a>
            <span className="spicon sphelp">
                <Img className="pointer icon" src={`/football/info/images/icon_help.gif${cacheVersion}`} onClick={(event) => { callBetTypes(getBettingGuidePath(this.props.oddsType, false), event) }} title={jsBettingGuide} />
            </span>
        </a>
    }
}
...
class AddBetBtn extends React.Component {
    render() {
        let addBetBtn = <a href={`javascript:addslip('');`} className="addslip poolDetails">
            <Img src={`/info/include/images/${jsLang}/btn_addslip.gif${cacheVersion}`} alt={jsaddSlip} title={jsaddSlip} className={`imgAddSlip`} />
        </a>;
        ...
        if (this.props.position != undefined && this.props.position != null && this.props.position == "footer") {
            return <div className={`footerAddBet`}>
                {addBetBtn}
